...
namespace ResearchProject.Controllers
{
    public class CheckoutController : Controller
    {
        ...
        public CheckoutController(CoffeeShopProjectContext context, INotyfService notyfService, IConfiguration config)
        {
            _context = context;
        ...
        public async Task<IActionResult> Index(BuyVM buy)
        {
            ...
            if (accountId != null)
            {
                var khh = _context.Customers.AsNoTracking().SingleOrDefault(x => x.CustomerId == Convert.ToInt32(accountId));
                model.CustomerId = khh.CustomerId;
                model.FullName = khh.FullName;
                model.Email = khh.Email;
                model.Phone = khh.Phone;
                model.Address = khh.Address;
