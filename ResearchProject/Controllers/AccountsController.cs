...
namespace ResearchProject.Controllers
{
    ...
    public class AccountsController : Controller
    {
        ...
        public AccountsController(CoffeeShopProjectContext context, INotyfService notyfService)
        {
            _context = context;
            _notyfService = notyfService;
        }

        //Validate Phone is existed or not
        [HttpGet]
        [AllowAnonymous]
        public IActionResult ValidatePhone(string Phone)
        {
            try
            {
                var cs = _context.Customers.AsNoTracking().SingleOrDefault(x => x.Phone.ToLower() == Phone.ToLower());
                if(cs != null)
        ...
        public IActionResult ValidateEmail(string Email)
        {
            try
            {
                var cs = _context.Customers.AsNoTracking().SingleOrDefault(x => x.Email.ToLower() == Email.ToLower());
                if (cs != null)
        ...
        public async Task<IActionResult> LoginAccount(LoginViewModel cs, string returnUrl = null)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    var kh = _context.Customers.AsNoTracking().SingleOrDefault(x => x.Email.Trim() == cs.UserName);
    ...
    public IActionResult ChangePassword(ChangePasswordViewModel model)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    var tk = _context.Customers.Find(Convert.ToInt32(accountId));
